...
class LoginHandler(BaseHandler):
    ...
    def get(self):
        ...
        if self.user:
            self.redirect_to('/settings/profile')
        params = {}
...
class SocialLoginHandler(BaseHandler):
    ...
    def get(self, provider_name):
        ...
        if provider_name in models.SocialUser.open_id_providers():
...
class RegisterHandler(BaseHandler):
    ...
    def post(self):
        ...
        if not user[0]: #user is a tuple
            ...
            self.add_message(message, 'error')
            return self.redirect_to('register',message)
        else:
            ...
            try:
                if not user[1].activated:
                    ...
                    return self.render_template('/registersuccess.html')
                ...
                self.add_message(message, 'success')
                return self.render_template('/register/success')
            except (AttributeError, KeyError), e:
...
class EditProfileHandler(BaseHandler):
    ...
    def get(self):
        ...
        if self.user:
            ...
            try:
                cred_info = SpreadSheetUser.get_by_user_id((user_info.key))
                try:
                    self.form.miner_pool.data = cred_info.miners[0].split(',')[0]
                    self.form.miner_nickname.data = cred_info.miners[0].split(',')[1]
                    self.form.miner_token.data = cred_info.miners[0].split(',')[2]
                    params['miner_pool'] = cred_info.miners[0].split(',')[0]
                    if len(cred_info.miners) == 2:
                        self.form.miner_pool_b.data = cred_info.miners[1].split(',')[0]
                        self.form.miner_nickname_b.data = cred_info.miners[1].split(',')[1]
                        self.form.miner_token_b.data = cred_info.miners[1].split(',')[2]
                        params['miner_pool_b'] = cred_info.miners[1].split(',')[0]
                    if len(cred_info.miners) == 3:
                        self.form.miner_pool_b.data = cred_info.miners[1].split(',')[0]
                        self.form.miner_nickname_b.data = cred_info.miners[1].split(',')[1]
                        self.form.miner_token_b.data = cred_info.miners[1].split(',')[2]
                        params['miner_pool_b'] = cred_info.miners[1].split(',')[0]
                        self.form.miner_pool_c.data = cred_info.miners[2].split(',')[0]
                        self.form.miner_nickname_c.data = cred_info.miners[2].split(',')[1]
                        self.form.miner_token_c.data = cred_info.miners[2].split(',')[2]
                        params['miner_pool_c'] = cred_info.miners[2].split(',')[0]
                except:
                    print 'update miner params failed'
                if cred_info.status == '1':
                    params['statusfree'] = True
                if cred_info.status == '2':
                    params['statuspremium'] = True
            except:
                params['status'] = False
    ...
    def post(self):
        ...
        country = self.form.country.data
        tz = self.form.tz.data
        miner_pool = self.form.miner_pool.data
        miner_nickname = self.form.miner_nickname.data.strip()
        miner_token = self.form.miner_token.data.strip()
        user_info = models.User.get_by_id(long(self.user_id))
        cred_info = SpreadSheetUser.get_by_user_id((user_info.key))
        if cred_info.status == '2':
            miner_pool_b = self.form.miner_pool_b.data
            miner_nickname_b = self.form.miner_nickname_b.data.strip()
            miner_token_b = self.form.miner_token_b.data.strip()
            miner_pool_c = self.form.miner_pool_c.data
            miner_nickname_c = self.form.miner_nickname_c.data.strip()
            miner_token_c = self.form.miner_token_c.data.strip()
        try:
            user_info = models.User.get_by_id(long(self.user_id))
            ...
            try:
                ...
                try:
                    if miner_pool != '""':
                        cred_info = SpreadSheetUser.get_by_user_id((user_info.key))
                        try:
                            cred_info.miners[0] = (('%s,%s,%s'%(miner_pool,miner_nickname,miner_token)))
                        except:
                            logging.error("Miner A failed to update")
                        if cred_info.status == '2':
                            #we should do this when we update from premium
                            if len(cred_info.miners) == 1:
                                cred_info.miners.append(" , , ")
                                cred_info.miners.append(" , , ")
                            try:
                                cred_info.miners[1] = (('%s,%s,%s'%(miner_pool_b,miner_nickname_b,miner_token_b)))
                            except:
                                logging.error("Miner B failed to update")
                            try:
                                cred_info.miners[2] = (('%s,%s,%s'%(miner_pool_c,miner_nickname_c,miner_token_c)))
                            except:
                                logging.error('Miner C Failed to update')
                        cred_info.put()
                except:
                    pass
